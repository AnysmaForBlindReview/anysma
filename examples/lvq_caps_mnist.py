...
if __name__ == "__main__":
    ...
    if not args.testing:
        ...
        test(model, (x_val, y_val), args, 'test on test dataset')


#### results
# job 895: loss: 0.0087 - lvq_caps_loss: 5.5612e-04 - decoder_loss: 0.0164 - lvq_caps_acc: 0.9999 - val_loss: 0.0231 - val_lvq_caps_loss: 0.0146 - val_decoder_loss: 0.0171 - val_lvq_caps_acc: 0.9967
# job 907: test on affnist: Test acc: 0.976390625

# ### visualization reconstruction on affnist
# import matplotlib.pyplot as plt
# import matplotlib.gridspec as gridspec
# from mpl_toolkits.axes_grid1 import make_axes_locatable
...
#
# plt.subplot2grid((1,12), (0,0), colspan=2)
# ax = plt.gca()
# img = plt.imshow(x_test[idx,:,:,0])
# img.set_cmap('gray')
# ax.set_xticks([])
...
# ax.set_title('input')
#
# plt.subplot2grid((1,12), (0,2), colspan=2)
# out = models.Model(model.input[0], model.get_layer('activation_5').output).predict(x_test[idx:idx+1])
# ax = plt.gca()
# img = ax.imshow(out[0,:,:,0])
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.03)
...
# ax.set_title('d_in Caps1')
#
# plt.subplot2grid((1,12), (0,4), colspan=2)
# out = np.reshape(models.Model(model.input[0], model.get_layer('gibbs_routing_1').output[1]).predict(x_test[idx:idx+1]), (-1, 8, 8, 1))
# ax = plt.gca()
# img = ax.imshow(out[0,:,:,0])
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.03)
...
# ax.set_title('d_in Caps2')
#
# plt.subplot2grid((1,12), (0,6), colspan=2)
# out = np.reshape(models.Model(model.input[0], model.get_layer('gibbs_routing_2').output[1]).predict(x_test[idx:idx+1]), (-1, 4, 4, 1))
# ax = plt.gca()
# img = ax.imshow(out[0,:,:,0])
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.03)
...
# ax.set_title('d_in Caps3')
#
# plt.subplot2grid((1,12), (0,8))
# out = np.reshape(models.Model(model.input[0], model.get_layer('gibbs_routing_3').get_output_at(0)[1]).predict(x_test[idx:idx+1]), (-1, 10, 1, 1))
# ax = plt.gca()
...
#
#
# plt.subplot2grid((1,12), (0,9))
# out = np.reshape(models.Model(model.input[0], model.get_layer('lvq_caps').get_output_at(0)[2]).predict(x_test[idx:idx+1]), (-1, 10, 1, 1))
# ax = plt.gca()
# img = ax.imshow(np.tile(out[0,:,:,0], (1,4)))
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="10%", pad=0.03)
...
# ax.set_title('p_out')
#
# x_recon = models.Model(eval_model.input, eval_model.get_layer('decoder').get_output_at(2)).predict(x_test[idx:idx+1,:,:,:], batch_size=16)
#
# plt.subplot2grid((1,12), (0,10), colspan=2)
# ax = plt.gca()
# img = plt.imshow(x_recon[0,:,:,0])
# img.set_cmap('gray')
# ax.set_xticks([])
...
# fig.set_size_inches(w=10,h=1.7)
# plt.show()
# plt.savefig('./vis_heatmap_recon_' + str(idx) + '.png')
...
# out = models.Model(model.input[0], model.get_layer('gibbs_routing_2').get_output_at(0)[1]).predict(x_train[:1000], batch_size=16)
#
...
# ######### random_walk final layer
#
# from matplotlib import pyplot as plt
#
# capsnet = model.get_layer('lvq_caps')
# param_in = layers.Input((10, 128))
# y = layers.Input(shape=(10,))
# masked_by_y = Mask()([param_in, y])
# output = decoder(masked_by_y)
#
# constructor = models.Model([param_in, y], output)
# fig = plt.figure(1)
#
# diss = model.get_layer('restricted_tangent_distance_1')
# protos, tangents, bounds = diss.get_weights()
...
#     for j in range(num):
#         out = out_ideal
#         for _, i in enumerate((0,)):
#             out[0][0, i, :] = protos[i, :] + np.matmul(tangents[i, :], (np.random.rand(16) - 0.5) * 10)
#             out[1][0, i] = 0
#
#         out2 = model_td2.predict([np.reshape(out[0], (1, 4, 4, 64)), np.reshape(out[1], (1, 4, 4, 1))])
#
#         y_true = np.zeros((1, 10))
#         y_true[:, digit] = 1
#         img = constructor.predict([out2[0], y_true])
#         images[idx * 28: (idx + 1) * 28, j * 28: (j + 1) * 28] = np.reshape(img, [28, 28])
#
# img = plt.imshow(images)
# ax = plt.gca()
...
# images = np.zeros((28 * 10, 28 * num*2))
# for idx in range(10):
#     j = np.where(np.argmax(y_test, 1) == idx)[0]
#     for i in range(0, num*2, 2):
#         jj = np.random.randint(0, len(j))
#         sample = x_test[j[jj]:j[jj]+1, :]
#         y_pred, x_recon = eval_model.predict(sample, batch_size=16)
#
#         images[idx * 28: (idx + 1) * 28, i * 28: (i + 1) * 28] = np.reshape(sample, [28, 28])
#         images[idx * 28: (idx + 1) * 28, (i + 1) * 28: (i + 2) * 28] = np.reshape(x_recon, [28, 28])
#
